...
define([], function() {return function() {require(['jquery', 'domReady!'], function($) {
	$(document).on('click', '.more, .less', function() {
		var $this = $(this);
		var $siblings;
		// 2020-06-09 I check `location` just for performance.
		if (0 === location.pathname.indexOf('/checkout/') && $this.closest('#shopping-cart-table').length) {
			// 2020-06-09
		...
		else if ($this.closest('#minicart-content-wrapper').length) {
			// 2020-06-08
